...
{
#ifdef BSD_PTYS
	fprintf(stderr, "%s_%s_%d:[%s]\n",__FILE__, __func__, __LINE__, "begin  BSD_PYS");
	...
	fprintf(stderr, "%s_%s_%d:flags = [%d]\n",__FILE__, __func__, __LINE__, flags | O_NONBLOCK);
    pty->master_fd = posix_openpt(flags | O_NONBLOCK);
#else
	printf("%s_%s_%d:flags = [%d]\n",__FILE__, __func__, __LINE__, flags);
	...
	fprintf(stderr, "%s_%s_%d:pty->name = [%s]\n",__FILE__, __func__, __LINE__, pty->name);
#endif
